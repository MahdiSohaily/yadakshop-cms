<!DOCTYPE html>
<html lang="fa" dir="rtl">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="shortcut icon" href="<?= asset_url('img/favicon.ico') ?>" type="image/x-icon">
    <title><?= $page_title ?></title>
    <link rel="stylesheet" href="<?= asset_url('css/assets/persian-datepicker.min.css') ?>">
    <link rel="stylesheet" href="<?= asset_url('css/output.css') ?>">
    <script src="<?= asset_url('js/assets/axios.js') ?>"></script>
    <script src="<?= asset_url('js/assets/jquery.min.js') ?>"></script>
    <script src="<?= asset_url('js/assets/persian-date.min.js') ?>"></script>
    <script src="<?= asset_url('js/assets/persian-datepicker.min.js') ?>"></script>
    <script src="<?= asset_url('js/helpers.js') ?>"></script>
</head>

<body class="min-h-fit bg-gray-100 dark:bg-gray-900">